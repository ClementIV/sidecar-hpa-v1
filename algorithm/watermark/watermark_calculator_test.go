...
func (tc *replicaCalcTestCase) runTest(t *testing.T) {
	...
	replicaCalculator := NewWatermarkReplicaCalculator(mClient, informer.Lister())
	...
	var replicaCalculation util.ReplicaCalculation
	var err error
	if tc.metric.spec.Resource != nil {
		// Resource metric tests
		// Update with the correct labels.
		replicaCalculation, err = replicaCalculator.GetResourceReplicas(logf.Log, tc.scale, tc.metric.spec, tc.shpa)
		...
		replicaCalculation, err = replicaCalculator.GetExternalMetricReplicas(logf.Log, tc.scale, tc.metric.spec, tc.shpa)
		if tc.expectedError != nil {
			require.Error(t, err, "there should be an error calculating the replica count")
			assert.Contains(t, err.Error(), tc.expectedError.Error(), "the error message should have contained the expected error message")
			return
...
func TestReplicaCalcDisjointResourcesMetrics(t *testing.T) {
	logf.SetLogger(logf.ZapLogger(true))
	metric1 := v1.MetricSpec{
		Type: v1.ResourceMetricSourceType,
		Resource: &v1.ResourceMetricSource{
			Name:           "deadbeef",
...
func TestReplicaCalcAboveAbsoluteExternal_Upscale1(t *testing.T) {
	...
	metric1 := v1.MetricSpec{
		Type: v1.ExternalMetricSourceType,
		External: &v1.ExternalMetricSource{
			MetricName:     "deadbeef",
			MetricSelector: &metav1.LabelSelector{MatchLabels: map[string]string{"foo": "bar"}},
...
func TestGetReadyPodsCount(t *testing.T) {
	...
	for _, f := range tests {
		t.Run(f.name, func(t *testing.T) {
			...
			replicaCalculator := NewWatermarkReplicaCalculator(nil, informer.Lister())
			...
			val, err := replicaCalculator.(*WatermarkCal).getReadyPodsCount(tc.scale, labels.SelectorFromSet(f.selector), readinessDelay*time.Second)
